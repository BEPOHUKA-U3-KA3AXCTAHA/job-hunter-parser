...
│   │       ├── migrations/
│   │       │   ├── env.py                # imports tables/ so every Row is registered with Base.metadata
│   │       │   └── versions/             # autogenerated revisions
│   │       └── tables/                   # ★ unified DB schema — one place for cross-module FKs
...
live under `app/infra/db/tables/{companies,people,applies}.py` so cross-module
...
ORM tables for all modules live in `app/infra/db/tables/`, not under any
module's `adapters/`.
...
**Infra-import rule:** `app.infra.db.tables.*` (Row classes, sessions, raw SQL) may be imported only by **adapters** and **entrypoints** (composition roots). `models/`, `ports/`, and `services/` must stay table-blind — they speak in domain entities, not in `Row` objects. This keeps the hexagon's inside (domain + use-cases) decoupled from the persistence layer.
