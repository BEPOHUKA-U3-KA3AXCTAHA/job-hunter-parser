...
def test_rule3_catches_adapter_folder_without_matching_port(lint_arch):
    ...
    assert any("Rule 3" in msg and "widgets" in msg for _, msg in lint_arch.violations), (
...
def test_rule3_catches_adapter_class_not_inheriting_from_port(lint_arch):
    ...
    (adapters / "sqla.py").write_text("class SqlaOrders(SomethingElse): ...\n")

    lint_arch.check_rule_3()
    assert any("Rule 3" in msg and "SqlaOrders" in msg for _, msg in lint_arch.violations), (
...
def test_rule3_passes_when_class_inherits_from_port(lint_arch):
    ...
    adapters = m / "adapters" / "orders"
    adapters.mkdir()
    (adapters / "sqla.py").write_text("class SqlaOrders(OrdersRepository): ...\n")
...
def test_rule4_catches_direct_adapter_import_across_modules(lint_arch):
    ...
    assert any("Rule 4" in msg and "shop.adapters" in msg for _, msg in lint_arch.violations), (
...
def test_rule4_allows_public_package_import_across_modules(lint_arch):
    ...
    services = other / "services"
    services.mkdir()
    (services / "do_thing.py").write_text("from app.modules.shop import OrdersRepository\n")

    lint_arch.check_rule_4()
    assert not lint_arch.violations, (
        f"Rule 4 false-positive on a public-package import. Violations: {lint_arch.violations}"
    )
...
def test_rule6_catches_tablename_outside_infra_tables(lint_arch):
    ...
    (m / "rogue.py").write_text("class Foo:\n    __tablename__ = 'foo'\n")

    lint_arch.check_rule_6()
    assert any("Rule 6" in msg and "__tablename__" in msg for _, msg in lint_arch.violations), (
...
def test_rule6_catches_base_subclass_outside_infra_tables(lint_arch):
    ...
    lint_arch.check_rule_6()
    assert any("Rule 6" in msg and "Foo" in msg for _, msg in lint_arch.violations), (
...
def test_rule7_catches_session_commit_in_service(lint_arch):
    ...
    services = m / "services"
    services.mkdir()
    (services / "place_order.py").write_text("async def go(session):\n    await session.commit()\n")

    lint_arch.check_rule_7()
    assert any("Rule 7" in msg for _, msg in lint_arch.violations), (
...
def test_rule7_catches_get_session_maker_outside_uow(lint_arch):
    ...
    lint_arch.check_rule_7()
    assert any("Rule 7" in msg for _, msg in lint_arch.violations), (
        f"Rule 7 missed get_session_maker in a service. Violations: {lint_arch.violations}"
    )
